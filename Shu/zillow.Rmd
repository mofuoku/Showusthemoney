...
### Sample Set
```{r}
dim(sample) # 90275 obs., 3 variables
head(sample,5)
...
A large number of records in the roomcnt column is marked as "0". This does not make sense, so we impute missing values as "bathroomcnt + bedroomcnt + 1". According to the complete data in the roomcnt column, the roomcnt is 1 room more than the sum of bathroomcnt and bedroomcnt in average. 
```{r}
completrm <- properties_clean[properties_clean$roomcnt!=0 & !is.na(properties_clean$roomcnt),]
addnum <- completrm$roomcnt-completrm$bathroomcnt-completrm$bedroomcnt
mean(addnum) # The average added number should be 1.

# roomcnt Imputation (impute 0 for missing datas)
properties_clean$roomcnt[is.na(properties_clean$roomcnt)]=0
...
Next, let’s join the two tables together on the parcelid column such that we only include properties that have a response value (logerror) in train.
```{r}
...
categorical <- categorical[-4] # rm "calculatedfinishedsquarefeet"

numeric_ <- names(train_set)[-which(names(train_set) %in% categorical)]

#numeric_ <- numeric_[-8] # rm assessmentyear
numeric_ <- numeric_[-c(4,5)] #rm "propertycountylandusecode","propertyzoningdesc"

categorical <- categorical[-13] # rm assessmentyear
...
train_set <- train_set[,c(8,1:7,9:24)]
#summary(train_set)
#View(train_set)
```
...
#num_prop_zip <- train_set %>%
  #group_by(regionidzip) %>%
  #summarise(numpropperzip = n())

#train_set$numzip = num_prop_zip$numpropperzip[match(train_set_nonscaled$regionidzip,num_prop_zip$regionidzip)]
...
```{r}
write.csv(train_set, 'train_set_nonscaled.csv',row.names = FALSE)
```
